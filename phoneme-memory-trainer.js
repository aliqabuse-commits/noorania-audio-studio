...
// مدرب الذاكرة الإدراكية اللونية للحروف — V1
// ================================
...
async function trainPhonemeMemory(phonemeKey) {
  ...
  try {
    ...
    for (const unit of memory.trainingUnits) {
    ...
    const identity =
      buildPerceptualIdentity(memory, samples);
...
function buildPerceptualIdentity(memory, samples) {
  ...
  const identity = {
    method: "Phoneme Color Memory Trainer V1",
...
function extractPerceptualFeatures(samples, sampleRate) {
  ...
  const active =
    detectMemoryActiveRange(samples);

  const activeSamples =
    samples.slice(active.start, active.end);

  const energy =
    calcMemoryRms(activeSamples);

  const zcr =
    calcMemoryZcr(activeSamples);

  const spectrum =
    memorySpectrum(activeSamples, sampleRate);

  const centroid =
    memorySpectralCentroid(spectrum);

  const spread =
    memorySpectralSpread(spectrum, centroid);

  const burstiness =
    calcMemoryBurstiness(activeSamples, sampleRate);
...
function memorySpectrum(segment, sampleRate) {
  ...
  const size =
    nextPowerOfTwoMemory(segment.length);
...
function calcPerceptualConfidence(values) {
  ...
  const centroidScore =
    1 / (
      1 +
      varianceMemory(values.centroidValues) / 100000
    );

  const spreadScore =
    1 / (
      1 +
      varianceMemory(values.spreadValues) / 100000
    );

  const energyScore =
    1 / (
      1 +
      varianceMemory(values.energyValues) * 100
    );

  const zcrScore =
    1 / (
      1 +
      varianceMemory(values.zcrValues) * 100
    );

  const burstScore =
    1 / (
      1 +
      varianceMemory(values.burstValues)
    );
...
function renderPhonemeMemoryReport(identity) {
  ...
  box.innerHTML = `
    ...
    <div>
      الهدف:
      ${identity.concept.goal}
    </div>

    <div>
      القاعدة:
      ${identity.concept.rule}
    </div>
  `;
...
function getAudioPromiseForMemory(key) {
