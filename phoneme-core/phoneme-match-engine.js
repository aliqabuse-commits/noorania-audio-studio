// ================================ // phoneme-match-engine.js // محرك الفصل بالجِينوم المركزي // يقارن عينة الاختبار عبر: // 1) الجينوم المركزي // 2) العائلة الإدراكية // 3) الختم الطيفي داخل الجينوم // 4) الذاكرة الإدراكية المستقلة حسب حالة الحركة // ملتزم بمرجع المسميات السيادي // ================================
 
console.log("🎯 phoneme-match-engine.js جاهز");
 
function getAvailablePhonemeKeysForMatch() { if ( typeof PHONEME_LETTER_DEFINITIONS !== "undefined" && PHONEME_LETTER_DEFINITIONS ) { return Object.keys(PHONEME_LETTER_DEFINITIONS); }
...
function resolveArabicSpokenInput(input) { const text = String(input || "").trim();
...
return total; } function compareSummaryWithFamilyAwareGenome( summary, genome, familyContext ) { if (!summary || !genome) { return Infinity; }
 
let weights = { energy: 1.0, centroid: 1.5, spread: 1.2, zcr: 0.8, burstEnergy: 1.3, burstCentroid: 1.4, burstSpread: 1.1, energyMovement: 1.1, spectralMovement: 1.5 };
 
const familyName = familyContext?.family || familyContext?.familyKey || "";
 
// ====================================== // عائلة الانفجار الشفوي // باء / ميم / واو ... // ======================================
 
if (familyName.includes("lip")) { weights.burstEnergy = 2.5; weights.energyMovement = 2.0; weights.centroid = 1.0; }
 
// ====================================== // عائلة الاستعلاء // قاف / صاد / طاء / ظاء ... // ======================================
 
if ( familyName.includes("isti") || familyName.includes("elevated") || familyName.includes("deep") ) { weights.centroid = 3.0; weights.burstCentroid = 3.0; weights.spectralMovement = 2.5; }
 
// ====================================== // الغنة // ======================================
 
if (familyName.includes("ghunna")) { weights.energy = 2.0; weights.spread = 2.0; weights.zcr = 0.5; }
...
function scoreSpectralSealDistance(summary, genome) { const seal = genome?.spectralSeal;
...
function resolveTargetStateForMatch(targetKey) { const map = { fatha: "fatha", kasra: "kasra", damma: "damma", sukoon_fatha: "sukoon_fatha", sukoon_kasra: "sukoon_kasra", sukoon_damma: "sukoon_damma" };
 
for (const state in map) { if (String(targetKey || "").includes(state)) { return map[state]; } }
 
return null; }
 
function scorePerceptualMemoryDistance( summary, perceptualMemory ) { if ( !summary || !perceptualMemory || !perceptualMemory.perceptualSignature ) { return 0; }
...
function scorePerceptualMemoryDistanceByState( summary, perceptualMemory, targetState ) { if ( !summary || !perceptualMemory || !perceptualMemory.perceptualSignatureByState || !targetState ) { return scorePerceptualMemoryDistance(summary, perceptualMemory); }
 
const stateMap = perceptualMemory.perceptualSignatureByState;
 
const stateKey = Object.keys(stateMap).find(function (key) { return key.includes(targetState); });
 
if (!stateKey || !stateMap[stateKey]) { return scorePerceptualMemoryDistance(summary, perceptualMemory); }
 
const sig = stateMap[stateKey]; if ( String(sig.text || "").includes("ْ") || String(stateKey || "").includes("sukoon") ) { return; } let total = 0;
 
total += weightedNormalizedDistance(summary.meanCentroid, sig.centroid?.mean, sig.centroid?.variance, 1.8); total += weightedNormalizedDistance(summary.meanSpread, sig.spread?.mean, sig.spread?.variance, 1.4); total += weightedNormalizedDistance(summary.meanEnergy, sig.energy?.mean, sig.energy?.variance, 1.2); total += weightedNormalizedDistance(summary.meanZcr, sig.zcr?.mean, sig.zcr?.variance, 1.1); total += weightedNormalizedDistance(summary.duration, sig.duration?.mean, sig.duration?.variance, 1.2);
 
return total; }
 
function scorePerceptualMemoryBestState(summary, perceptualMemory) { if ( !summary || !perceptualMemory || !perceptualMemory.perceptualSignatureByState ) { return { distance: scorePerceptualMemoryDistance(summary, perceptualMemory), stateKey: null, stateText: null, stateRole: null, confidence: 0 }; }
...
const best = scores[0] || null; const second = scores[1] || null;
 
if (!best) { return { distance: scorePerceptualMemoryDistance(summary, perceptualMemory), stateKey: null, stateText: null, stateRole: null, confidence: 0 }; }
 
const stateMargin = second ? second.distance - best.distance : 0;
 
return { distance: best.distance, stateKey: best.stateKey, stateText: best.stateText, stateRole: best.stateRole, stateMargin: Number(stateMargin.toFixed(4)), confidence: second ? Number((stateMargin / Math.max(best.distance, 0.0001)).toFixed(4)) : 1, allStateScores: scores }; }
 
function weightedNormalizedDistance(value, mean, variance, weight) { value = Number(value || 0); mean = Number(mean || 0); variance = Number(variance || 0); weight = Number(weight || 1);
 
const tolerance = Math.sqrt(variance) || Math.abs(mean) * 0.15 || 1;
 
return ( Math.abs(value - mean) / tolerance ) * weight; }
 
function classifySeparationDecision(winner, second, margin) { if (!second) { return { label: "غير كافٍ للمقارنة", note: "يوجد جينوم واحد فقط، لذلك لا نستطيع قياس الفصل الحقيقي." }; }
 
const ratio = second.distance / Math.max(winner.distance, 0.0001);
 
if (margin > 3 || ratio > 1.6) { return { label: "فصل قوي ✅", note: "العينة بعيدة بوضوح عن الحرف الثاني، وهذا مؤشر جيد على أن الجينوم يساعد في الفصل." }; }
 
if (margin > 1 || ratio > 1.25) { return { label: "فصل متوسط ⚠️", note: "العينة أقرب إلى الحرف الفائز، لكن المسافة ليست كبيرة بما يكفي للاطمئنان الكامل." }; }
 
return { label: "فصل ملتبس ❌", note: "الجينومات قريبة من العينة. نحتاج تحسين التسجيل أو تقوية الجينوم أو إضافة طبقات مقارنة زمنية أقوى." }; }
 
function askActualSpokenKey() { const answer = prompt( "ما الحرف الذي نطقته فعليًا؟\n\n" + "اكتب الحرف بحركته مثل:\n\n" + "بَ\n" + "صِ\n" + "طُ" );
 
if (!answer) return null;
 
const parsed = resolveArabicSpokenInput(answer);
 
if (!parsed.key) { alert("لم أتعرف على الحرف. اكتب مثل: بَ أو صِ أو طُ"); return null; }
 
if (!parsed.state) { alert("اكتب الحرف مع الحركة: فتحة أو كسرة أو ضمة."); return null; }
 
return parsed; }
 
function saveCognitiveMatchResult( buttonKey, actualKey, winner, results, decision, margin ) { const logKey = "cognitive_match_results_log";
 
const oldLog = JSON.parse( localStorage.getItem(logKey) || "[]" );
 
oldLog.push({ buttonKey: buttonKey, actualKey: actualKey, detectedKey: winner.key, detectedLabel: winner.label, detectedPhoneme: winner.phoneme, isCorrect: actualKey === winner.key, margin: Number(margin.toFixed(4)), decision: decision.label, results: results.map(function (r) { return { key: r.key, label: r.label, phoneme: r.phoneme, distance: Number(r.distance.toFixed(4)) }; }), createdAt: new Date().toISOString() });
...
function renderMatchResultsLog(filterKey) { const raw = localStorage.getItem("cognitive_match_results_log");
 
const allResults = JSON.parse(raw || "[]");
 
const results = filterKey ? allResults.filter(function (r) { return r.buttonKey === filterKey; }) : allResults;
 
const correctResults = results.filter(function (r) { return r.actualKey === r.detectedKey; });
 
const accuracy = results.length ? ((correctResults.length / results.length) * 100).toFixed(2) : "0.00";
 
const avgCorrectMargin = correctResults.length ? ( correctResults.reduce(function (sum, r) { return sum + Number(r.margin || 0); }, 0) / correctResults.length ).toFixed(4) : "0.0000";
...
function clearCognitiveMatchResultsLog() { localStorage.removeItem("cognitive_match_results_log");
