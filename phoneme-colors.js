...
// ======================================
// تعريف قاموس الألوان الإدراكية للحروف
// تنبيه مهم: يجب عدم تغيير المفاتيح (Keys) البرمجية هنا
// لأنها مرتبطة ارتباطاً وثيقاً بذاكرة النظام والجينوم المركزي
// أي تغيير سيؤدي إلى فقدان الارتباط اللوني الإدراكي للحرف.
// ======================================
...
function normalizePhonemeColorKey(key) {
  if (!key) return null;

  const cleanKey = String(key).trim();

  return PHONEME_COLOR_ALIASES[cleanKey] || cleanKey;
}


function getPhonemeColor(key) {
  const normalizedKey = normalizePhonemeColorKey(key);

  return PHONEME_COLORS[normalizedKey] || null;
}
...
function bindPhonemeToColor(phonemeKey) {
  const normalizedKey = normalizePhonemeColorKey(phonemeKey);

  const data = PHONEME_COLORS[normalizedKey];
