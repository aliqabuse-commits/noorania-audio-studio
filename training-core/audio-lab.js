...
const GENOME_REGIONS = [
  { id: "preCarrier", label: "ما قبل الحامل", color: "rgba(148,163,184,0.30)" },
  { id: "carrier", label: "الحامل", color: "rgba(250,204,21,0.35)" },
  { id: "payload", label: "المحمول", color: "rgba(0,242,255,0.35)" },
  { id: "tail", label: "الذيل", color: "rgba(248,113,113,0.30)" }
];
...
function initWaveform(blob) {
  ...
  if (WaveSurfer.Spectrogram && document.getElementById("spectrogram")) {
...
function createSmartGenomeRegions() {
  ...
  const firstSound = findFirstAbove(smoothed, soundThreshold, 2);
  const firstStrong = findFirstAbove(smoothed, strongThreshold, 2);
  const lastStrong = findLastAbove(smoothed, strongThreshold, 2);
  const lastSound = findLastAbove(smoothed, soundThreshold, 2);
...
async function playPurifiedPayload() {
  try {
    const saved = localStorage.getItem("ba_purified_payload_result");
    ...
    const currentKey = filenameEl ? filenameEl.innerText.trim() : null;
    ...
    const decoded = await decodeBlobToMonoForPlayback(blob);
...
async function decodeBlobToMonoForPlayback(blob) {
...
function buildGenome(referenceKey) {
  ...
  return {
    ...
    version: "genome-v3-smart-noise-aware-experiment",
